...
def event_details(event_id):
    ...
    checkin_methods = db.execute_query('''
        SELECT 
            check_in_method,
            COUNT(*) as count
        FROM attendance
...
def attendance_data(event_id):
    ...
    current_attendance = db.execute_query('''
        SELECT 
            u.id,
            u.full_name,
            u.email,
...
def event_reports(event_id):
    ...
    attendance_timeline = db.execute_query('''
        SELECT 
    ...
    # Networking stats
    networking_stats = db.execute_query('''
    ...
    # Top networkers
    top_networkers = db.execute_query('''
        SELECT 
            u.full_name,
...
def networking_analytics():
    ...
    networking_by_event = db.execute_query('''
        SELECT 
            e.title,
            e.id,
...
print(f"\n{Colors.CYAN}Creating Event Admin Controller...{Colors.END}\n")

filepath = 'src/controllers/event_admin_controller.py'
os.makedirs(os.path.dirname(filepath), exist_ok=True)

with open(filepath, 'w', encoding='utf-8') as f:
    f.write(EVENT_ADMIN_CONTROLLER.strip())

print(f"{Colors.GREEN}✓{Colors.END} Created: {Colors.CYAN}{filepath}{Colors.END}")
print(f"\n{Colors.GREEN}✅ Event Admin Controller created!{Colors.END}\n")