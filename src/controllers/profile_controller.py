...
def delete_qualification(qual_id):
    ...
    if qual and qual['user_id'] == session['user_id']:
        execute_query("DELETE FROM qualifications WHERE id = %s", (qual_id,))
...
def my_nfc():
    ...
    # Generate NFC code
    nfc_code = generate_user_nfc_code(user['id'], user['email'])
...
def refresh_nfc():
    '''Refresh NFC code'''
    from src.controllers.nfc_controller import generate_user_nfc_code
