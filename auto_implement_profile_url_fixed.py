...
if os.path.exists(scanner_path):
    ...
    # Find and replace processScan function directly
    if 'function processScan' in content:
        print("   🔧 Replacing processScan function...")
        
        # Find the entire processScan function
        start_marker = 'function processScan(scannedData, method) {'
        end_marker = 'function showScanResult'
        
        if start_marker in content and end_marker in content:
            start_pos = content.find(start_marker)
            end_pos = content.find(end_marker)
            
            # New processScan function with all URL format handling
            new_process_scan = '''function processScan(scannedData, method) {
    let userId = null;
...
'''
            ...
            content = content[:start_pos] + new_process_scan + content[end_pos:]
